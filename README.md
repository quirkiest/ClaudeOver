...
"Hey Jibo …" goes to Claude. **Hold his head for two seconds** (or swipe down, or say "Claude off") to get normal Jibo back.
...
```
 Jibo menu ──tap──▶ ClaudeOver skill ──POST /v1/takeover {on}──▶ gateway (Docker, LAN)
  ...
  OFF = head hold · swipe down · "Claude off" · 30 min idle   → Claude → Jibo speaks
```
...
4. Hold his head for two seconds to exit.
